...
class CommentArea extends Component<CommentAreaProps, CommentAreaState> {
  ...
  handleSubmit = async (e: FormEvent) => {
    ...
    if (this.state.userInput) {
      try {
        // No sanitization is applied here; raw user input is sent
        const response = await fetch(`/users/${this.props.userId}/weak_post`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          ...
          body: JSON.stringify({
            title: 'Comment Post',
            content: this.state.userInput, // Vulnerable input
          }),
        });
        ...
        this.setState({ comments, userInput: '' });
      } catch (error) {
